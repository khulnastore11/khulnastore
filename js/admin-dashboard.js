...
onAuthStateChanged(auth, (user) => {
  ...
  if (user.email !== "khulnastore11@gmail.com") {
...
async function renderProducts() {
  ...
  snapshot.forEach(docSnap => {
    ...
    list.innerHTML += `
      <div class="cart-item">
  <img src="${p.images?.[p.featuredIndex || 0]}" class="cart-img">
...
document.getElementById("featuredIndex").value =
  product.featuredIndex || 0;
...
async function handleSubmit() {
  ...
  const featuredIndex = Number(
    document.getElementById("featuredIndex").value || 0
  );
...
<br>
  ...
  <select data-id="${id}" class="status-select custom-status">
